...
gosu zotonic /usr/local/bin/zotonic_config.awk -v defaults=true /home/zotonic/.zotonic/0.12/zotonic.config > $TMPFILE
gosu zotonic cat $TMPFILE > /home/zotonic/.zotonic/0.12/zotonic.config && rm $TMPFILE
...
if [[ -z $DBHOST ]]; then
    DBHOST=$(awk -F'"' '/.{dbhost,/{print $2}' /home/zotonic/.zotonic/0.12/zotonic.config)
    if [[ $DBHOST ]]; then
	echo "export DBHOST=$DBHOST">>/home/zotonic/.bashrc
    fi
fi
if [[ -z $DBPORT ]]; then
    DBPORT=$(awk -F'[,}]' '/.{dbport,/{print $2}' /home/zotonic/.zotonic/0.12/zotonic.config)
    if [[ $DBPORT ]]; then
